...
(function (global) {
  ...
  const PART_LABELS = { force_together: "강제결합", force_separate: "강제분리", conflict: "갈등분리", positive: "긍정유지",
  ...
  function blockHtml(m, marker, isLeader, inPool, pinned) {
    ...
    return `<div class="${cls}" data-id="${esc(m.id)}" title="${esc(m.issue_note || "")}">
      <div class="b-top"><span class="mbti">${esc(m.mbti || "–")}</span><span class="b-name">${esc(m.name)}</span>${cand}${iss}${pin}${crown}</div>
  ...
  function exportCsv() {
    ...
    const byId = Store.byId();
    let csv = "﻿composition,team,team_name,team_leader,team_note,id,name,mbti,primary_disp,secondary_disp,comp_avg,leadership,issue_level,issue_note,leader_candidate,pinned\n";
    p.compositions.forEach((c) => {
      ...
      c.teams.forEach((ids, ti) => ids.forEach((id) => {
        ...
        const cells = [c.name, ti + 1, names[ti] || ("팀 " + (ti + 1)), leaders[ti] === id ? "Y" : "", notes[ti] || "",
          ...
          m.leadership == null ? "" : m.leadership, m.issue_level || 0, m.issue_note || "", TB.leaderCandidate(m) ? "Y" : "", pins[id] != null ? "Y" : ""];
