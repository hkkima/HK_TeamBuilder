...
- **① 데이터 탭 (엑셀 대체)**: 표에서 학생을 **인라인 추가/수정/삭제**. 역량은 **드롭다운**(리더십·관리=상/중/하, 기획·작업·소통=매우우수~매우미흡), 성향 드롭다운, 이슈·MBTI, **특별/멘탈/건강/매몰 체크**,
...
- **② 편성 탭 (피그마 대체 + 자동화)**:
  ...
  - **카테고리 태그(멘탈/건강/매몰)**: 같은 카테고리는 **한 팀 2명 이상 강하게 회피**(가능하면 항상 0), 서로 다른 카테고리끼리는 **약하게 회피**. 문제 패널·[자동 해결] 지원.
...
3. **카테고리 태그 관리** — 같은 카테고리(멘탈/건강/매몰) 한 팀 2명↑ 강하게 회피, 교차는 약하게
...
| `special`(특수 관리) |  | Y/1이면 특수 관리 태그 — **한 팀에 2명 이상 절대 금지(하드)** |
| `mental`/`health`/`sunk` |  | 멘탈/건강/매몰 카테고리. 같은 카테고리 한 팀 2명↑ 강하게 회피(소프트) |
...
| `--w-flag-same` | 60 | 같은 카테고리(멘탈/건강/매몰) 한 팀 2명↑ 회피(강) |
