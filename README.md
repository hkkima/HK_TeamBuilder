...
- **② 편성 탭 (피그마 대체 + 자동화)**:
  ...
  - **팀장/부팀장 자동 지정 + 단계별 편성**: 자동 편성은 ① **팀장·부팀장**(후보=책임자/매니저, 쌍에 **책임자≥1·매니저≥1**) → ② **분위기메이커 확보**(팀 성향점수≥1, 주=1·부=0.5) → ③ 나머지 순으로 구성합니다. 각 팀의 팀장(👑)·부팀장(🎖)이 자동 지정되고 **리스트 최상단에 팀장, 그 아래 부팀장** 순으로 표시됩니다. 부족한 팀은 헤더·문제 패널에 경고(최선 배치).
