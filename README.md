...
- **② 편성 탭 (피그마 대체 + 자동화)**:
  ...
  - **문제 패널**: 특수관리 겹침·갈등쌍·고이슈 겹침·팀장쌍/분위기메이커 부족·리더 없는 팀을 목록화하고 **[자동 해결]** 원클릭 수정.
  - **팀장/부팀장 자동 지정 + 단계별 편성**: 자동 편성은 ① **팀장·부팀장**(후보=책임자/매니저, 쌍에 **책임자≥1·매니저≥1**) → ② **분위기메이커 확보**(팀 성향점수≥1, 주=1·부=0.5) → ③ 나머지 순으로 구성합니다. 각 팀의 팀장(👑)·부팀장(🎖)이 자동 지정되고 **리스트 최상단에 팀장, 그 아래 부팀장** 순으로 표시됩니다. 팀장은 쌍 중 **복합 적합도(책임자·매니저 성향 + 리더십 + 관리 능력, 관리대상은 감점)**가 높은 쪽입니다. 부족한 팀은 헤더·문제 패널에 경고(최선 배치).
  - **특수 관리 태그**: 데이터 그리드의 `특별` 체크(또는 CSV `special=Y`). 태그된 학생은 **한 팀에 2명 이상 절대 배치 안 됨**(자동 편성 하드 보장, 수동 겹침 시 최우선 문제로 경고).
  - **성향 분포(핵심)**: 자동 편성의 핵심 목표는 **① 갈등 분리 ② 성향 분포**입니다. 같은 주 성향이 한 팀에 몰리지 않도록(혼합도) 강하게 유도합니다.
  - **카테고리 태그(멘탈/매몰)**: **운영자 확인용 표시**입니다. 자동 편성 점수에는 **반영하지 않으며**(기본 가중치 0), 겹치면 문제 패널에 안내만 합니다(원하면 [자동 해결]로 수동 조정 가능). ※ **특수 관리(특별)** 태그는 그대로 하드 격리.
  - **필수 역할**: 팀마다 **분위기메이커·매니저·책임자 각 1명 이상**(강한 소프트), **서포터는 보너스**. 부족 시 팀 헤더·문제 패널로 경고.
  - 추천안은 **조합 목록**에서 골라 하나만 표시, 손으로 고친 편성을 **저장**. 팀장(👑)·부팀장(🎖)·팀 이름·메모 지원(팀장/부팀장은 블럭 버튼으로 수동 변경 가능).
- **③ 비교 탭**: 조합들을 나란히 두고 **점수 항목 델타 표**로 선택 근거 확인.
- **CSV 내보내기**: 목록의 모든 조합을 `composition,team,team_name,team_role,team_note,id,name,mbti,primary_disp,…,issue_level,…,pinned` 형식으로 저장(`team_role`=팀장/부팀장).
...
3. **카테고리 태그(멘탈/매몰)** — 운영자 확인용 표시만(자동 편성 미반영). 특수 관리(특별)는 하드 격리 유지
...
| `mental`/`sunk` |  | 멘탈/매몰 카테고리. **운영자 확인용 표시**(자동 편성 미반영, 기본 가중치 0) |
...
| `--w-flag-same` | 0 | (운영자 확인용) 같은 카테고리 격리 — 기본 미반영 |
| `--w-flag-cross` | 0 | (운영자 확인용) 카테고리 교차 — 기본 미반영 |
| `--w-leader-pair` | 35 | 팀장+부팀장 쌍이 책임자≥1·매니저≥1 커버 |
| `--w-mood-cover` | 22 | 팀 분위기메이커 점수(주1/부0.5)≥1 |
...
| `--w-disp-diversity` | 45 | **성향 분포(혼합도)** — 같은 주 성향 몰림 방지(핵심) |
