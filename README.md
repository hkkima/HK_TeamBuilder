...
> 성향 6종(작업자·매니저·분위기메이커·책임자·연구원·서포터), 역량 5차원(리더십·관리·기획·작업·소통)
> **이슈 강도(1–3)**, 단원별 교과 점수(60–100, 별도 CSV·역량 보조), 그리고 방향성 **관계 평가(POS/NEG + 가중치 W)**.

- **웹 프론트(GitHub Pages)** + **Python CLI** 두 가지 사용 방식 제공
- 설치 불필요: Python 3.9+ 표준 라이브러리만 사용 (pandas 등 의존성 없음)
- 입력 CSV(**수강생 마스터** / **관계 평가** / **교과 점수**) + 선택(강제 규칙), 출력은 리포트 + CSV
...
- **① 데이터 탭 (엑셀 대체)**: 표에서 학생을 **인라인 추가/수정/삭제**. 역량은 **드롭다운**(리더십·관리=상/중/하, 기획·작업·소통=매우우수~매우미흡), 성향 드롭다운, 이슈·MBTI, **관리대상(특별)/멘탈/매몰 체크**, 교과평균(단원 점수) 표시.
  관계는 `FROM 👍/👎 TO · W` 행으로 편집, 강제 규칙도 선택 UI. **CSV 파일 끌어놓기** 임포트(수강생/관계/교과 각각) + CSV 내보내기.
- **② 편성 탭 (피그마 대체 + 자동화)**:
  - **드래그&드롭**(터치·마우스·키보드) 으로 블럭을 팀 사이 이동 — 옮길 때마다 점수/문제 즉시 갱신.
  - **핀/락**: 블럭 📌 또는 팀 🔒로 고정 → **자동 편성**이 고정은 유지하고 나머지만 재배치(부분 재편성).
  - **문제 패널**: 특수관리 겹침·갈등쌍·고이슈 겹침·리더 없는 팀을 목록화하고 **[자동 해결]** 원클릭 수정.
  - **특수 관리 태그**: 데이터 그리드의 `특별` 체크(또는 CSV `special=Y`). 태그된 학생은 **한 팀에 2명 이상 절대 배치 안 됨**(자동 편성 하드 보장, 수동 겹침 시 최우선 문제로 경고).
  - **카테고리 태그(멘탈/매몰)**: 같은 카테고리는 **한 팀 2명 이상 강하게 회피**(가능하면 항상 0), 서로 다른 카테고리끼리는 **약하게 회피**. 문제 패널·[자동 해결] 지원.
  - **필수 역할**: 팀마다 **분위기메이커·매니저·책임자 각 1명 이상**(강한 소프트), **서포터는 보너스**. 부족 시 팀 헤더·문제 패널로 경고.
...
0. **특수 관리 태그 격리** — 태그된 학생은 한 팀에 2명 이상 절대 금지 (**하드 보장**)
1. **갈등 분리** — NEG 관계(가중치 W 강도 반영)를 다른 팀으로 (사실상 하드)
2. **긍정 유지** — POS 관계(W 강도 반영)를 일부 같은 팀에
3. **카테고리 태그 관리** — 같은 카테고리(멘탈/매몰) 한 팀 2명↑ 강하게 회피, 교차는 약하게
4. **이슈 관리** — 고이슈(강도 ≥2) 학생끼리 같은 팀 회피 + 팀 간 이슈 총량 균등 분산
5. **필수 역할 + 성향/리더** — 팀마다 분위기메이커·매니저·책임자 각 1명(서포터 보너스), 성향 다양성, 리더 확보
6. **역량(보조)** — 팀 평균 평준화 + 핵심역량(기획·작업·소통) 커버리지
...
| `primary_disp`(주 성향) |  | 작업자·매니저·분위기메이커·책임자·연구원·서포터 |
| `secondary_disp`(부 성향) |  | 위 6종 |
| `mbti` |  | 예: INTP |
| `issue_level`(이슈 강도) |  | 0~3 (0/빈칸=없음). ≥2면 격리·분산 대상 |
| `issue_note`(이슈 비고) |  | 자유 텍스트 |
| `special`(관리 대상/특별) |  | Y/1이면 특수 관리 태그 — **한 팀에 2명 이상 절대 금지(하드)**. 별칭: 관리대상/특별 |
| `mental`/`sunk` |  | 멘탈/매몰 카테고리. 같은 카테고리 한 팀 2명↑ 강하게 회피(소프트) |

> 교과 점수(단원별)는 **별도 CSV**(`grades_template.csv`)로 받습니다 — 아래 참조.
...
### `grades_template.csv` — 교과 점수 (별도 파일)
단원별 교과 점수(60~100)를 학생 마스터와 **분리해서** 받습니다. 이름 또는 `INDEX`(=id)로 join.

| 컬럼 | 필수 | 설명 |
|---|---|---|
| `INDEX` |  | 학생 id (없으면 `이름`으로 join) |
| `이름` | ✅ | 이름 (join 키) |
| `1단원`~`4단원` / `unit*` |  | 단원별 점수(60~100, 자동 감지). **역량(보조) 평준화의 유일한 기준** |
| `평균` |  | 참고용(내부 계산은 단원 점수로 자동 산출) |

> CLI: `--grades grades.csv`. 웹: 데이터 탭에서 교과 CSV를 끌어놓으면 단원 점수가 학생에 병합됩니다.
...
| `--w-positive` | 8 | 긍정 유지 |
| `--w-special-stack` | 500 | 특수 관리 태그 격리(하드 안전망) |
| `--w-flag-same` | 60 | 같은 카테고리(멘탈/매몰) 한 팀 2명↑ 회피(강) |
| `--w-flag-cross` | 4 | 서로 다른 카테고리 한 팀 혼재 회피(약) |
| `--w-role-required` | 25 | 팀마다 분위기메이커·매니저·책임자 각 1명 |
| `--w-role-supporter` | 5 | 팀에 서포터 있으면 보너스 |
